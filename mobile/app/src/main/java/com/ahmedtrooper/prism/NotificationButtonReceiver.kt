...
class NotificationButtonReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        when (intent.action) {
            "$PREFIX.PLAY_PAUSE" -> PrismLib.command(arrayOf("cycle", "pause"))
            "$PREFIX.ACTION_PREV" -> PrismLib.command(arrayOf("playlist-prev"))
            "$PREFIX.ACTION_NEXT" -> PrismLib.command(arrayOf("playlist-next"))
        }
    }
    ...
    companion object {
        ...
        private const val PREFIX = "com.ahmedtrooper.prism"
    }
}
