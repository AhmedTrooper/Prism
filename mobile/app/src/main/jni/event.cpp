...
static void sendPropertyUpdateToJava(JNIEnv *env, mpv_event_property *prop)
{
    ...
    case MPV_FORMAT_NONE:
        env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_eventProperty_S, jprop);
        break;
    case MPV_FORMAT_FLAG:
        env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_eventProperty_Sb, jprop,
            (jboolean) (*(int*)prop->data != 0));
        break;
    case MPV_FORMAT_INT64:
        env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_eventProperty_Sl, jprop,
            (jlong) *(int64_t*)prop->data);
        break;
    case MPV_FORMAT_DOUBLE:
        env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_eventProperty_Sd, jprop,
            (jdouble) *(double*)prop->data);
        break;
    case MPV_FORMAT_STRING:
        jvalue = env->NewStringUTF(*(const char**)prop->data);
        env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_eventProperty_SS, jprop, jvalue);
        break;
    default:
...
{
    env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_event, event);
}
...
static void sendLogMessageToJava(JNIEnv *env, mpv_event_log_message *msg)
{
    ...
    env->CallStaticVoidMethod(prism_PrismLib, prism_PrismLib_logMessage_SiS,
