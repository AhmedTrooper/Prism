...
import com.ahmedtrooper.prism.PrismLib.MpvEvent
import kotlin.math.*
...
class PlayerActivity : AppCompatActivity(), PrismLib.EventObserver, TouchGesturesObserver {
    ...
    private fun initListeners() {
        with (binding) {
            ...
            rewindBtn.setOnClickListener {
                PrismLib.command(arrayOf("seek", "-10", "relative"))
            ...
            zoomCard.setOnClickListener { resetZoom() }
            abLoopBtn.setOnClickListener { toggleABLoop() }
            abLoopBtn.setOnLongClickListener { clearABLoop(); true }
    ...
    override fun onNewIntent(intent: Intent?) {
        ...
        if (!activityIsForeground && didResumeBackgroundPlayback) {
            if (this.newIntentReplace) {
                PrismLib.command(arrayOf("loadfile", filepath, "replace"))
                showToast(getString(R.string.notice_file_play))
            } else {
                PrismLib.command(arrayOf("loadfile", filepath, "append"))
                showToast(getString(R.string.notice_file_appended))
            }
            moveTaskToBack(true)
        } else {
            PrismLib.command(arrayOf("loadfile", filepath))
        }
    }

    private fun updateAudioPresence() {
        val haveAudio = PrismLib.getPropertyBoolean("current-tracks/audio/selected")
        if (haveAudio == null) {
        ...
        isPlayingAudio = (haveAudio && PrismLib.getPropertyBoolean("mute") != true)
    }

    private fun isPlayingAudioOnly(): Boolean {
        if (!isPlayingAudio)
            return false
        val image = PrismLib.getPropertyString("current-tracks/video/image")
    ...
    private fun savePosition() {
        ...
        val curPath = PrismLib.getPropertyString("path") ?: ""
        ...
        if (!shouldSavePosition)
            return
        if (PrismLib.getPropertyBoolean("eof-reached") ?: true) {
            Log.d(TAG, "player indicates EOF, not saving watch-later config")
            return
        }
        PrismLib.command(arrayOf("write-watch-later-config"))
    }
    ...
    private fun onAudioFocusChange(type: Int, source: String) {
        ...
        when (type) {
            ...
            AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK -> {
                PrismLib.command(arrayOf("multiply", "volume", AUDIO_FOCUS_DUCKING.toString()))
                audioFocusRestore = {
                    val inv = 1f / AUDIO_FOCUS_DUCKING
                    PrismLib.command(arrayOf("multiply", "volume", inv.toString()))
                }
            }
    ...
    private fun pauseForDialog(): StateRestoreCallback {
        ...
        if (useKeepOpen) {
            ...
            val oldValue = PrismLib.getPropertyString("keep-open")
            PrismLib.setPropertyBoolean("keep-open", true)
            return {
                oldValue?.also { PrismLib.setPropertyString("keep-open", it) }
            }
        }
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        binding.controls.updateLayoutParams<MarginLayoutParams> {
            bottomMargin = if (!controlsAtBottom) {
                Utils.convertDp(this@PlayerActivity, 60f)
            } else {
                0
            }
            leftMargin = if (!controlsAtBottom) {
                Utils.convertDp(this@PlayerActivity, if (isLandscape) 60f else 24f)
            } else {
                0
            }
    ...
    private fun playlistPrev() = PrismLib.command(arrayOf("playlist-prev"))
    private fun playlistNext() = PrismLib.command(arrayOf("playlist-next"))
    ...
    private fun openPlaylistMenu(restore: StateRestoreCallback) {
        ...
        impl.listeners = object : PlaylistDialog.Listeners {
            private fun openFilePicker(skip: Int) {
                openFilePickerFor(RCODE_LOAD_FILE, "", skip) { result, data ->
                    if (result == RESULT_OK) {
                        ...
                        PrismLib.command(arrayOf("loadfile", path, "append"))
                        impl.refresh()
                    }
                }
            }
            ...
            override fun openUrl() {
                val helper = Utils.OpenUrlDialog(this@PlayerActivity)
                with (helper) {
                    builder.setPositiveButton(R.string.dialog_ok) { _, _ ->
                        PrismLib.command(arrayOf("loadfile", helper.text, "append"))
                        impl.refresh()
                    }
            ...
            override fun onItemPicked(item: PrismView.PlaylistItem) {
                PrismLib.setPropertyInt("playlist-pos", item.index)
                dialog.dismiss()
            }
        }
    ...
    private fun openTopMenu() {
        ...
        fun addExternalThing(cmd: String, result: Int, data: Intent?) {
            ...
            val path2 = if (path.startsWith("content://"))
                ...
                path
            PrismLib.command(arrayOf(cmd, path2, "cached"))
        }
        ...
        val buttons: MutableList<MenuItem> = mutableListOf(
                ...
                MenuItem(R.id.chapterPrev) {
                    PrismLib.command(arrayOf("add", "chapter", "-1")); true
                },
        ...
        if ((PrismLib.getPropertyInt("chapter-list/count") ?: 0) == 0)
    ...
    private fun genericPickerDialog(
        ...
        val dialog = with(MaterialAlertDialogBuilder(this)) {
            ...
            setPositiveButton(R.string.dialog_ok) { _, _ ->
                picker.number?.let {
                    if (picker.isInteger())
                        PrismLib.setPropertyInt(property, it.toInt())
                    else
                        PrismLib.setPropertyDouble(property, it)
                }
            }
        ...
        picker.number = PrismLib.getPropertyDouble(property)
        dialog.show()
    }
    ...
    private fun openAdvancedMenu(restoreState: StateRestoreCallback) {
        ...
        val buttons: MutableList<MenuItem> = mutableListOf(
                MenuItem(R.id.subSeekPrev) {
                    PrismLib.command(arrayOf("sub-seek", "-1")); true
                },
                MenuItem(R.id.subSeekNext) {
                    PrismLib.command(arrayOf("sub-seek", "1")); true
                },
                MenuItem(R.id.statsBtn) {
                    PrismLib.command(arrayOf("script-binding", "stats/display-stats-toggle")); true
                },
                    ...
                    with (MaterialAlertDialogBuilder(this)) {
                        setItems(R.array.aspect_ratio_names) { dialog, item ->
                            if (ratios[item] == "panscan") {
                                ...
                                PrismLib.setPropertyDouble("panscan", 1.0)
                            } else {
                                PrismLib.setPropertyString("video-aspect-override", ratios[item])
                                PrismLib.setPropertyDouble("panscan", 0.0)
                            }
                            dialog.dismiss()
                        }
                        setOnDismissListener { restoreState() }
        ...
        for (i in 1..3) {
            buttons.add(MenuItem(statsButtons[i-1]) {
                PrismLib.command(arrayOf("script-binding", "stats/display-page-$i")); true
            })
        }
    ...
    private fun cycleAspectRatio() {
        ...
        if (ratio == "stretch") {
            ...
            PrismLib.setPropertyString("video-aspect-override", screenAspect.toString())
            PrismLib.setPropertyDouble("panscan", 0.0)
        } else {
            PrismLib.setPropertyString("video-aspect-override", ratio)
            PrismLib.setPropertyDouble("panscan", panscanVal)
        }
        showToast(name, true)
    }
    ...
    private fun openFilePickerFor(requestCode: Int, title: String, skip: Int?, callback: ActivityResultCallback) {
        ...
        val path = PrismLib.getPropertyString("path") ?: ""
        if (path.startsWith('/'))
    ...
    private val mediaSessionCallback = object : MediaSessionCompat.Callback() {
        ...
        override fun onSetRepeatMode(repeatMode: Int) {
            PrismLib.setPropertyString("loop-playlist",
                if (repeatMode == PlaybackStateCompat.REPEAT_MODE_ALL) "inf" else "no")
            PrismLib.setPropertyString("loop-file",
    ...
    override fun event(eventId: Int) {
        ...
        if (eventId == MpvEvent.MPV_EVENT_START_FILE) {
            ...
            currentPanY = 0.0
            abLoop.clear()
            updateABLoopButton(ABLoopController.State(null, null))
    ...
    private fun resetZoom() {
        ...
        currentPanX = 0.0
        currentPanY = 0.0
        PrismLib.setPropertyDouble("video-zoom", 0.0)
        PrismLib.setPropertyDouble("video-pan-x", 0.0)
        PrismLib.setPropertyDouble("video-pan-y", 0.0)
        PrismLib.setPropertyDouble("video-scale-x", 1.0)
        PrismLib.setPropertyDouble("video-scale-y", 1.0)
    ...
    private fun toggleABLoop() {
        val pos = psc.positionSec.toDouble()
        val state = abLoop.toggle(pos)
        showToast(abLoop.toastText(state), true)
        updateABLoopButton(state)
    }
    private fun clearABLoop() {
        abLoop.clear()
        showToast("A-B: off", true)
        updateABLoopButton(ABLoopController.State(null, null))
    }
    private fun updateABLoopButton(state: ABLoopController.State = abLoop.getState()) {
        binding.abLoopBtn.text = when {
            state.a == null -> "A-B"
            state.b == null -> "A: ${Utils.prettyTime(state.a.toInt())}"
            else -> "AB"
        }
        binding.abLoopBtn.alpha = if (state.a != null) 1f else 0.7f
    }
        ...
        when (p) {
            ...
            PropertyChange.Seek -> {
                ...
                if (smoothSeekGesture) {
                    ...
                    PrismLib.command(arrayOf("seek", "$newPosExact", "absolute+keyframes"))
                }
            ...
            PropertyChange.Volume -> {
                ...
                if (targetStep > maxVolume) {
                    ...
                    PrismLib.setPropertyInt("volume", totalBoost)
                    ...
                    PrismLib.setPropertyInt("volume", 100)
            ...
            PropertyChange.SeekFixed -> {
                ...
                PrismLib.command(arrayOf("seek", seekTime.toString(), "relative"))
            ...
            PropertyChange.Custom -> {
                ...
                PrismLib.command(arrayOf("keypress", "0x%x".format(keycode)))
            }
            ...
            PropertyChange.Zoom -> {
                if (diff > 0f) {
                    ...
                    PrismLib.setPropertyDouble("video-zoom", currentZoom)
            ...
            PropertyChange.Pan -> {
                if (currentZoom > 0.02) {
                    ...
                    PrismLib.setPropertyDouble("video-pan-x", currentPanX)
                    PrismLib.setPropertyDouble("video-pan-y", currentPanY)
                }
            }
    ...
    companion object {
        ...
        private const val RESULT_INTENT = "com.ahmedtrooper.prism.PlayerActivity.result"
