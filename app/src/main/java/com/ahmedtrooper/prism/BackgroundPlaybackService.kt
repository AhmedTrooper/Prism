...
class BackgroundPlaybackService : Service(), PrismLib.EventObserver {
    override fun onCreate() {
        thumbnailHandler = Handler(mainLooper)

        PrismLib.addObserver(this)
    }
    ...
    private fun buildNotification(): Notification {
        ...
        with (builder) {
            ...
            setSmallIcon(R.drawable.ic_prism_symbolic)
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        paused = PrismLib.getPropertyBoolean("pause") == true
        shouldShowPrevNext = (PrismLib.getPropertyInt("playlist-count") ?: 0) > 1
    ...
    companion object {
        ...
        /* Set by PlayerActivity; for connecting the notification to the media session */
        var mediaToken: MediaSessionCompat.Token? = null
        /* Set by PlayerActivity; to notify on thumbnail changes */
        ...
        private fun grabThumbnail() {
            val fmt = PrismLib.getPropertyString("video-format")
            thumbnail = if (fmt.isNullOrEmpty())
                null
            else
                PrismLib.grabThumbnail(THUMB_SIZE)
        }
